...
namespace Magentomobileshop\Pushnotification\Helper;
...
class Data extends AbstractHelper
{
    ...
    public function sendPushAndroid($registration_id, $message)
    {
        ...
        $baseUrl = $this->parseUrl($this->_storeManager->getStore()->getBaseUrl());
        $title   = preg_replace("/^([a-zA-Z0-9].*\.) ?
            ([a-zA-Z0-9][a-zA-Z0-9-]{1,61}[a-zA-Z0-9]\.[a-zA-Z.]{2,})$/", '$2', $baseUrl['host']);
        $msg =
            [
            'message' => $message,
            'title'   => $title,
    ...
    public function sendPushIOS($registration_id, $message)
    {
        ...
        $fileName   = 'AudioOnline_Dev.pem';
        $passphrase = $this->getPassPharas();
        $ctx        = stream_context_create();
        stream_context_set_option($ctx, 'ssl', 'local_cert', $path);
        stream_context_set_option($ctx, 'ssl', 'passphrase', $passphrase);

        $ios_mode = ($this->getMode() == 1) ?
        'ssl://gateway.sandbox.push.apple.com:2195' : 'ssl://gateway.push.apple.com:2195';
        // Open a connection to the APNS server
        //@codingStandardsIgnoreStart
        try {
            $fp = stream_socket_client($ios_mode, $err, $errstr, 60, STREAM_CLIENT_CONNECT |
            ...
            //@codingStandardsIgnoreEnd
            $result = $this->write($fp, $msg, strlen($msg));
        } catch (\Exception $e) {
            $error['code']  = 401;
            $error['error'] = $e->getMessage();
            return $error;
    ...
    public function getGoogleGcmKey()
    {
        return $this->scopeConfig->getValue(
            'magentomobileshop_pushnotification/android_notification/google_api_key',
    ...
    {
        return $this->scopeConfig->getValue(
            'magentomobileshop_pushnotification/android_notification/push_notification_status',
            \Magento\Store\Model\ScopeInterface::SCOPE_STORE
        );
    }
    ...
    {
        return $this->scopeConfig->getValue(
            'magentomobileshop_pushnotification/ios_notification/ios_push_notification_status',
            \Magento\Store\Model\ScopeInterface::SCOPE_STORE
        );
    }
    ...
    {
        return $this->scopeConfig->getValue(
            'magentomobileshop_pushnotification/ios_notification/passphras',
            \Magento\Store\Model\ScopeInterface::SCOPE_STORE
        );
    }
    ...
    {
        return $this->scopeConfig->getValue(
            'magentomobileshop_pushnotification/ios_notification/upload_pem',
            \Magento\Store\Model\ScopeInterface::SCOPE_STORE
        );
    }
    ...
    {
        return $this->scopeConfig->getValue(
            'magentomobileshop_pushnotification/ios_notification/push_notification_mode',
            \Magento\Store\Model\ScopeInterface::SCOPE_STORE
        );
    }
    ...
    {
        return $this->scopeConfig->getValue(
            'magentomobileshop_pushnotification/background_notification/background_notification',
            \Magento\Store\Model\ScopeInterface::SCOPE_STORE
        );
    }
        ...
        if (!empty($registrationId)) {
            if ($type == self::ANDROID_CODE) {
                if ($this->getAndroidNotificationStatus()) {
                    ...
                    $response['success']    = $this->total_android_success .
                        ' Android notification has been sent successfully.';
                    $response['error'] = $this->total_android_error .
                        ' Android notification has been failed.';
                    return $response;
                } else {
                    $response['statusCode'] = 402;
                    $response['type']       = $type;
    ...
    private function __validationChecks($data, $response, $message)
    {
        if ($this->getIosNotificationStatus()) {
            ...
            $response['type']       = $type;
            $response['msg']        = 'Notifications has been sent.';
            $response['success']    = $this->total_ios_error .
                ' Ios Notification has been sent successfully.';
            $response['error'] = $this->total_ios_success . ' Ios Notification has been failed.';
            return $response;
        } else {
            $response['statusCode'] = 402;
    ...
    private function __androidChecks($data, $response, $message)
    {
        if ($this->getAndroidNotificationStatus()) {
            foreach ($data as $key => $value) {
                ...
                if ($status == 401) {
                    $response['statusCode'] = 401;
                    $response['msg']        =
                        "Invalid (legacy) Server-key delivered
                                    or Sender is not authorized to perform request.";
                    return $response;
                }
            }
            $response['statusCode'] = 200;
